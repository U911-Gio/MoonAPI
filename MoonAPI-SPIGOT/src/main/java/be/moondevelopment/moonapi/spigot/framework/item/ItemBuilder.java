...
*/
package be.moondevelopment.moonapi.spigot.framework.item;
...
public class ItemBuilder {
    ...
    /** Initalizes the ItemBuilder with {@link org.bukkit.Material} and Amount and Short byte */
    public ItemBuilder(Material material, int amount, int type) {
        if(material == null) material = Material.AIR;
        if(((amount > material.getMaxStackSize()) || (amount <= 0)) && (!unsafeStackSize)) amount = 1;
        this.amount = amount;
        this.type = type;
        this.item = new ItemStack(material, amount, (byte) type);
        this.material = material;
    }
